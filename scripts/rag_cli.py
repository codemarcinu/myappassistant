...
async def add_file(args):
    ...
    if result["success"]:
        logger.info(f"Successfully processed file with {result['chunks_processed']} chunks")
    else:
...
async def add_directory(args):
    ...
    if result["success"]:
        logger.info(f"Successfully processed {result['successful_files']}/{result['files_processed']} files")
        ...
        logger.error(f"Failed to process directory: {result.get('error', 'Unknown error')}")
...
async def add_url(args):
    ...
    if result["success"]:
        logger.info(f"Successfully processed URL with {result['chunks_processed']} chunks")
    else:
...
def main():
    ...
    # Add file command
    file_parser = subparsers.add_parser("add-file", help="Add a file to the RAG knowledge base")
    ...
    file_parser.add_argument("--save", action="store_true", help="Save the vector store after processing")
    
    # Add directory command
    dir_parser = subparsers.add_parser("add-directory", help="Add all files in a directory")
    ...
    dir_parser.add_argument("--extensions", help="Comma-separated list of file extensions to include")
    dir_parser.add_argument("--batch-size", type=int, default=5, help="Number of files to process concurrently")
    dir_parser.add_argument("--save", action="store_true", help="Save the vector store after processing")
    ...
    url_parser.add_argument("--save", action="store_true", help="Save the vector store after processing")
    
    # Search command
    search_parser = subparsers.add_parser("search", help="Search the RAG knowledge base")
    search_parser.add_argument("query", help="Search query")
    search_parser.add_argument("--limit", type=int, default=5, help="Maximum number of results")
    search_parser.add_argument("--min-similarity", type=float, default=0.65, help="Minimum similarity threshold")
    search_parser.add_argument("--tags", help="Filter by comma-separated tags")
    search_parser.add_argument("--category", help="Filter by category")
    search_parser.add_argument("--full-text", action="store_true", help="Show full text of results")
