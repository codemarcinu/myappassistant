...
from src.backend.core.hybrid_llm_client import hybrid_llm_client, ModelComplexity
...
class EnhancedQASystem:
    ...
    async def initialize_knowledge_base(self):
        ...
        logger.info(f"Loaded {result.get('successful_files', 0)} documents with "
                   f"{result.get('total_chunks_processed', 0)} chunks")
        return True
        
    async def add_knowledge_source(self, source_type: str, source_path: str, metadata: Optional[Dict] = None):
        """
    ...
    async def answer_question(self, question: str, use_rag: bool = True) -> Dict[str, Any]:
        """
