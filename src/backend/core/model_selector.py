...
class ModelSelector:
    ...
    def select_model(
        ...
        if detected_language == "pl":
            logger.info("No suitable models found, falling back to default Polish model")
            return self.DEFAULT_POLISH_MODEL
        else:
            logger.info("No suitable models found, falling back to default international model")
            return self.DEFAULT_INTERNATIONAL_MODEL
