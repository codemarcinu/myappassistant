...
class MemoryEntry:
    ...
    def get_decay_factor(self, half_life_days: int = 30) -> float:
        ...
        adjusted_half_life = half_life_days * (1 + 0.1 * self.importance + 0.05 * self.access_count)
...
class LongTermMemory:
    ...
    async def retrieve_relevant(
        ...
        if self.use_faiss and len(self.entries) > 0:
            ...
            candidates = [(self.entries[idx], float(score)) for score, idx in zip(D[0], I[0])]
            candidates = [(entry, score) for entry, score in candidates if score >= min_similarity]
            
            # Re-rank by weighted importance
            candidates.sort(key=lambda x: x[0].get_weighted_score() * (1 - x[1]/10), reverse=True)
            ...
            results.sort(
                key=lambda x: x[0].get_weighted_score() * x[1], 
                reverse=True
            )
    ...
    async def _calculate_importance(self, text: str) -> float:
        ...
        try:
            response = await llm_client.chat(
                model="gemma3:12b",
                messages=[
                    {"role": "system", "content": "You evaluate the importance of information."},
    ...
    def _prune_memories(self) -> None:
        ...
        if (datetime.now() - self.last_cleanup) < timedelta(hours=1) and len(self.entries) < self.max_memories:
            return
        ...
        entries_to_remove = max(int(len(self.entries) * 0.2), len(self.entries) - int(self.max_memories * 0.8))
        
        if entries_to_remove > 0:
            # Get indices to remove (lowest scoring first)
            to_remove = [self.entries.index(entry) for entry, _ in scored_entries[:entries_to_remove]]
            ...
            if self.use_faiss and to_remove:
                ...
                if remaining_indices:
                    embeddings = np.array([self.entries[i].embedding for i in remaining_indices], dtype=np.float32)
...
class ConversationMemoryManager:
    ...
    async def add_message(self, role: str, content: str, importance_override: Optional[float] = None) -> None:
