...
import hashlib
import numpy as np
from typing import List, Dict, Any, Optional, Union, Tuple, Callable, AsyncGenerator
...
try:
    ...
    logging.warning("SentenceTransformers not available, falling back to LLM-based embeddings")
...
class RAGDocumentProcessor:
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        if self.use_local_embeddings and self.embedding_model_local:
            ...
            embedding = self.embedding_model_local.encode(text, convert_to_tensor=False).tolist()
            return embedding
        ...
        try:
            ...
            logger.warning(f"Failed to use Ollama for embeddings: {e}, falling back to hybrid_llm_client")
    ...
    async def process_document(
        ...
        for i, chunk in enumerate(chunks):
            ...
            if self.use_pinecone:
                # Store in Pinecone
                try:
                    await self.store_in_pinecone(chunk_id, embedding, chunk, chunk_metadata)
                    storage = "pinecone"
                except Exception as e:
    ...
    async def store_in_pinecone(
        ...
        await asyncio.to_thread(
            ...
            vectors=[(chunk_id, embedding, metadata_copy)]
        )
