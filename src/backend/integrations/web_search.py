...
# Constants
DEFAULT_CACHE_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "data", "search_cache")
...
class WebSearchClient:
    ...
    def _init_sources(self, sources_config: Optional[List[Dict[str, Any]]] = None) -> None:
        ...
        for src_config in config_to_use:
            ...
            if source.api_key:
                ...
                logger.warning(f"No API key found for {source.name} (env var: {source.api_key_env_var})")
    ...
    async def _make_request(
        ...
        while attempt < retries:
            try:
                ...
                logger.error(f"HTTP error {e.response.status_code} from {source.name}: {str(e)}")
                
                # Special handling for rate limiting
                if e.response.status_code == 429:
                    wait_time = int(e.response.headers.get('Retry-After', RETRY_DELAY * (2 ** attempt)))
                    logger.warning(f"Rate limited by {source.name}, waiting {wait_time}s")
            ...
            logger.info(f"Retrying {source.name} in {wait_time}s (attempt {attempt+1}/{retries})")
            await asyncio.sleep(wait_time)
            attempt += 1
        
        return None
    
    def _parse_newsapi_response(self, data: Dict[str, Any], query: str) -> SearchResponse:
