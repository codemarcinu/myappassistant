    ...
    async def process(self, input_data: Dict[str, Any]) -> AgentResponse:
        ...
        try:
            ...
            for provider in self.providers:
                ...
                try:
                    logger.info(f"Fetching weather data for {location} from {provider.name}")
                    if provider.name == "weatherapi":
                        weather_data = await self._fetch_weatherapi(location, validated_data.get("days", 3), include_alerts)
                    elif provider.name == "openweathermap":
                        weather_data = await self._fetch_openweathermap(location, validated_data.get("days", 3), include_alerts)
            ...
            if not weather_data:
                return self._handle_error(
                    ValueError(f"Nie udało się pobrać prognozy pogody dla {location} z żadnego dostępnego źródła.")
                )
            
            # Check for severe weather alerts
            has_severe_alerts = any(alert.severity >= 2 for alert in weather_data.alerts)
    ...
    async def _fetch_weatherapi(
        ...
        try:
            ...
            response = await self.http_client.get(
                f"{provider.base_url}/forecast.json", 
                params=params
            )
                ...
                for alert_data in data["alerts"]["alert"]:
                    ...
                    alerts.append(WeatherAlert(
                        ...
                        effective=datetime.fromisoformat(alert_data.get("effective", "").replace("Z", "+00:00")) 
                            if "effective" in alert_data else None,
                        expires=datetime.fromisoformat(alert_data.get("expires", "").replace("Z", "+00:00"))
                            if "expires" in alert_data else None
                    ))
    ...
    async def _fetch_openweathermap(
        ...
        try:
            ...
            current = {
                ...
                "wind_kph": current_data.get("wind", {}).get("speed") * 3.6,  # m/s to km/h
                ...
                for i, day in enumerate(forecast_data["daily"]):
                    ...
                    forecast.append({
                        ...
                        "day": {
                            ...
                            "condition": {
                                "text": day.get("weather", [{}])[0].get("description", "Unknown"),
            ...
            if include_alerts and "alerts" in forecast_data:
                for alert_data in forecast_data.get("alerts", []):
                    ...
                    alerts.append(WeatherAlert(
                        ...
                        effective=datetime.fromtimestamp(alert_data.get("start", 0)),
    ...
    def _format_response(
        ...
        if weather_data.alerts:
            ...
            for alert in weather_data.alerts:
                severity_text = "WYSOKI" if alert.severity >= 3 else "ŚREDNI" if alert.severity == 2 else "NISKI"
                weather_summary += f"- {alert.event} (Poziom: {severity_text}): {alert.headline}\n"
