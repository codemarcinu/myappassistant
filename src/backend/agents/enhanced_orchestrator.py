...
from ..models.user_profile import ProfileManager, InteractionType
from ..models.conversation import Conversation, Message
from .enhanced_base_agent import ImprovedBaseAgent, EnhancedAgentResponse, ErrorSeverity
...
class EnhancedOrchestrator:
    ...
    async def _get_memory_manager(self, session_id: str, conversation: Optional[Conversation] = None) -> ConversationMemoryManager:
    ...
    async def process_command(
        ...
        try:
            ...
            if len(user_command) < 15:
                suggestions = await self.profile_manager.get_personalized_suggestions(session_id)
                if suggestions:
            ...
            if agent_states:
                if agent_states.get("shopping", False) or agent_states.get("cooking", False):
            ...
            logger.info(f"Processed command in {processing_time:.2f}s (intent: {intent_data.get('intent', 'unknown')})")
...
"""
        ...
        model = "gemma2:2b" if complexity_level == ModelComplexity.SIMPLE else "gemma3:12b"
        
        response = await hybrid_llm_client.chat(
            messages=[
                {"role": "system", "content": "Jesteś asystentem analizującym intencje użytkownika."},
    ...
    async def _handle_clarification(self, command: str, intent_data: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    async def _handle_weather(self, command: str, session_id: str, complexity_level: ModelComplexity) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "response": agent_response.text or agent_response.message or "Prognoza pogody przetworzona pomyślnie.",
                "data": agent_response.data
            }
    ...
    async def _handle_search(self, command: str, session_id: str, complexity_level: ModelComplexity) -> Dict[str, Any]:
        ...
        try:
            ...
            search_terms = command.replace("wyszukaj", "").replace("znajdź", "").replace("szukaj", "").strip()
...
"""
            ...
            response = await hybrid_llm_client.chat(
                messages=[
                    {"role": "system", "content": "Jesteś pomocnym asystentem, który tworzy zwięzłe podsumowania wyników wyszukiwania."},
                    {"role": "user", "content": prompt}
                ],
                model="gemma3:12b",
    ...
    async def _handle_rag(self, command: str, session_id: str, complexity_level: ModelComplexity) -> Dict[str, Any]:
...
"""
            ...
            response = await hybrid_llm_client.chat(
                messages=[
                    {"role": "system", "content": "Jesteś pomocnym asystentem, który odpowiada na pytania w oparciu o dostarczone dokumenty."},
                    {"role": "user", "content": prompt}
                ],
                model="gemma3:12b",
    ...
    async def _handle_cooking(self, command: str, session_id: str, complexity_level: ModelComplexity) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "response": agent_response.text or "Zapytanie kulinarne przetworzone pomyślnie.",
                "data": agent_response.data
            }
        except Exception as e:
    ...
    async def _handle_shopping(self, command: str, session_id: str, complexity_level: ModelComplexity) -> Dict[str, Any]:
...
{context_text}"""
        ...
        response = await hybrid_llm_client.chat(
            messages=[
                {"role": "system", "content": "Jesteś pomocnym, przyjaznym asystentem, który odpowiada po polsku."},
                {"role": "user", "content": prompt}
            ],
            model=model,
            force_complexity=complexity_level
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            if hasattr(self.weather_agent, 'close') and callable(self.weather_agent.close):
