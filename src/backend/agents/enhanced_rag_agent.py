...
class EnhancedRAGAgent(BaseAgent):
        ...
        if not self.initialized:
            ...
            if not self.vector_store.chunks:
                ...
                if docs_dir.exists() and docs_dir.is_dir():
                    logger.info(f"Initializing RAG agent with documents from {docs_dir}")
            ...
            logger.info(f"RAG agent initialized with {len(self.vector_store.chunks)} chunks")
    ...
    async def add_document(
        ...
        """
        return await self.document_processor.process_document(content, source_id, metadata)
...
Answer:"""
        ...
        response = await hybrid_llm_client.chat(
            messages=[
                {"role": "system", "content": "You are a helpful assistant that answers questions based on the provided context. Include relevant details from the context, but don't add information that isn't there."},
