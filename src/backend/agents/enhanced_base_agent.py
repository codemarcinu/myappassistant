...
from typing import Any, AsyncGenerator, Dict, List, Optional, Tuple, Union, Generic, TypeVar
...
class ImprovedBaseAgent(ABC, Generic[T]):
        ...
        try:
            ...
            if self._should_alert(str(e), error_severity):
                await self._send_alert(
                    f"Error in {self.name}.{func.__name__}",
                    error_info,
                    error_severity
                )
    ...
    async def _multi_tiered_fallback(
        ...
        rewritten_response = await self._try_prompt_rewriting(input_data, original_error)
        ...
        if self.fallback_attempts < self.max_fallback_attempts:
            simplified_response = await self._try_simplified_model(input_data, original_error)
    ...
    async def _try_prompt_rewriting(
        ...
        try:
            ...
            response = await hybrid_llm_client.chat(
                messages=[
                    {"role": "system", "content": "Jesteś pomocnym asystentem przepisującym zapytania."},
    ...
    async def _try_simplified_model(
        ...
        try:
            ...
            response = await hybrid_llm_client.chat(
                messages=[
                    {"role": "system", "content": "Jesteś pomocnym asystentem udzielającym prostych odpowiedzi."},
    ...
    async def _send_alert(
        ...
        if self.alert_config.email_alerts and self.alert_config.email_recipients:
            logger.info(f"Would send email alert to {self.alert_config.email_recipients}")
