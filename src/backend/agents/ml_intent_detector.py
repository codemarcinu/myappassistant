...
from src.backend.orchestration_components import IntentData, MemoryContext # Importuj klasy bazowe

logger = logging.getLogger(__name__)

class BERTIntentDetector:
    def __init__(self, model_name: str = "distilbert-base-uncased", labels: Optional[List[str]] = None):
        self.model_name = model_name
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.classifier = None
        # Przykładowe etykiety intencji. W prawdziwej aplikacji pochodziłyby z danych treningowych.
        self.intent_labels = labels if labels is not None else [
            "cooking", "weather", "search", "document", 
            "shopping", "meal_plan", "analytics", "general"
        ]
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Initializing BERTIntentDetector on device: {device if device != -1 else 'cpu'}")
    ...
    async def detect_intent(self, text: str, context: MemoryContext) -> IntentData:
        ...
        if not self._is_initialized:
            ...
            if not self._is_initialized: # Ponowna weryfikacja po próbie inicjalizacji
                logger.error("BERTIntentDetector not initialized, falling back to simple detection.")
                return self._fallback_detection_simple(text)

        processed_text = self._preprocess_text(text, context)

        try:
            predictions = self.classifier(processed_text)[0] # pipeline zwraca listę list, bierzemy pierwszy wynik
            ...
            if max_confidence < 0.5: # Próg pewności
                 logger.warning(f"Low confidence for intent '{best_intent_type}' ({max_confidence:.2f}), falling back.")
                 return self._fallback_detection_simple(text)
            ...
            logger.error(f"Error during BERT intent detection: {e}. Falling back to simple detection.", exc_info=True)
            return self._fallback_detection_simple(text)

    def _preprocess_text(self, text: str, context: MemoryContext) -> str:
        """Przygotowuje tekst, dodając kontekst rozmowy, jeśli dostępny."""
        history_messages = [msg["content"] for msg in context.history if "content" in msg]
        if history_messages:
    ...
    def _map_predictions_to_intents(self, predictions: List[Dict[str, Any]]) -> Dict[str, float]:
