...
class TestEnhancedRAGAgent(unittest.TestCase):
    ...
    @patch(
        "backend.core.rag_document_processor.rag_document_processor.process_document"
    )
    async def test_add_document(self, mock_process_document):
        ...
        result = await self.agent.add_document(
            self.test_content, "test-doc-1", self.test_metadata
        )
    ...
    async def test_search(self, mock_search):
        ...
        mock_search.assert_called_once_with(
            query="vector databases", k=1, filter_metadata=None, min_similarity=0.65
        )
...
TestEnhancedRAGAgent.test_add_document = async_test(
    TestEnhancedRAGAgent.test_add_document
)
