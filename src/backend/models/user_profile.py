...
class UserProfile(Base):
    ...
    last_active = Column(DateTime(timezone=True), default=datetime.now, onupdate=datetime.now)
...
class ProfileManager:
    ...
    async def get_or_create_profile(self, session_id: str) -> UserProfileData:
        ...
        # Look up in database
        from ..core.crud import get_user_profile_by_session, create_user_profile
    ...
    async def update_preferences(
        ...
        # Update in database
        from ..core.crud import update_user_preferences
    ...
    async def update_schedule(
        self,
        session_id: str,
        schedule: Union[UserSchedule, Dict]
    ) -> UserProfileData:
        ...
        # Update in database
        from ..core.crud import update_user_schedule
    ...
    async def get_personalized_suggestions(
        ...
        if schedule.wake_time <= current_time_only <= time(hour=schedule.wake_time.hour+2, minute=0):
        ...
        if is_work_day and schedule.work_start_time <= current_time_only <= schedule.work_end_time:
            # Lunch time suggestion
            lunch_start = time(hour=schedule.lunch_time.hour-1, minute=schedule.lunch_time.minute)
            lunch_end = time(hour=schedule.lunch_time.hour+1, minute=schedule.lunch_time.minute)
            
            if lunch_start <= current_time_only <= lunch_end:
                suggestions.append("Czas na przerwę na lunch! Potrzebujesz przepisu na szybki posiłek?")
            
            # Work break reminder
            if len(suggestions) == 0 and current_time_only.hour % 2 == 0 and current_time_only.minute < 15:
    ...
    async def analyze_interests(self, session_id: str) -> List[str]:
        ...
        from ..core.crud import get_user_activities
        ...
        for activity in activities:
            if activity.interaction_type == InteractionType.QUERY.value and activity.content:
                # Extract possible interests from queries
                words = activity.content.lower().split()
                for word in words:
                    if len(word) > 3 and word not in ["czym", "jaki", "jaka", "jakie", "gdzie", "kiedy"]:
        ...
        # Find top interests
        sorted_interests = sorted(interest_counters.items(), key=lambda x: x[1], reverse=True)
        ...
        if top_interests:
            ...
            # Update in database
            from ..core.crud import update_user_topics
            await update_user_topics(
                self.db,
                profile_data.user_id,
                top_interests
            )
