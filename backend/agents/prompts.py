...
def get_intent_recognition_prompt(user_command: str, conversation_context: str = "") -> str:
    """
...
def get_entity_extraction_prompt(user_command: str, intent: str, conversation_context: str = "") -> str:
    """
...
def get_resolver_prompt(options: list, user_reply: str, conversation_context: str = "") -> str:
    """
    Generuje prompt do rozwiązywania niejednoznaczności w wyborze użytkownika.
    """
    base_prompt = """
    Twoim zadaniem jest analiza odpowiedzi użytkownika i dopasowanie jej do jednej z przedstawionych mu wcześniej opcji.
    Zwróć obiekt JSON z jednym kluczem: 'choice'. Indeks jest numerem opcji z listy (zaczynając od 1).
    Jeśli nie jesteś w stanie dopasować odpowiedzi, zwróć null.
