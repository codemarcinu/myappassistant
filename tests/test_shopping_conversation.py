...
async def db_session():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute("DELETE FROM shopping_trips")
...
async def add_test_data(db_session, products):
    ...
    for product in products:
        product.shopping_trip_id = trip.id
...
async def test_meal_planning_conversation(db_session, sample_products):
    ...
    # Mock the chef agent
    chef_response = AgentResponse(
        success=True,
        text="Z dostępnych produktów (chleb, mleko, jajka, jabłka, ser) mogę zaproponować następujące dania:\n1. Tosty z serem\n2. Jajecznica na grzankach\n3. Szarlotka",
        text_stream=AsyncMock(
            __aiter__=AsyncMock(
                return_value=[
    ...
    # Mock the meal planner agent
    with patch("backend.agents.chef_agent.ChefAgent.process", new_callable=AsyncMock) as mock_chef:
        mock_chef.return_value = chef_response
        
        # Initialize a real MealPlannerAgent but with mocked dependencies
